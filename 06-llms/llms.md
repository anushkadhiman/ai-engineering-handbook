...
- **Significance of Pre-training**
  ...
  - It's versatile. A pre-trained model acts as a "base" that can be adapted to many different applications (e.g., summarization, translation, code generation).
...
- **Significance of Fine-tuning**
  ...
  - It is efficient. It requires far less computational power and data than pre-training from scratch, as it builds upon existing knowledge.
...
- They act as a bridge between human language and computer-readable numerical data.
...
- Due to their straightforward architecture, they are computationally efficient to train and scale, allowing them to handle vast amounts of data.
...
- Models can inherit and amplify biases present in the training data.
...
- It is efficient. Proper tokenization ensures the model processes information faster by reducing the overall number of tokens.
...
- Delivers more relevant results by understanding the _"why"_ behind a search.

- Handles conversational, natural language queries effectively.

- **Cross-lingual Capability:** Can match concepts across different languages.

---

## How LLMs handle out-of-vocabulary (OOV) words?

LLMs handle out-of-vocabulary (OOV) words primarily by breaking them down into smaller, recognizable subword tokens (e.g., using Byte Pair Encoding - BPE), allowing the model to understand unseen words by analyzing their components. If a word is completely unknown, it may be broken down into individual characters or represented with special unknown token markers ().

**What are some key techniques for handling OOV Words?**

- Subword Tokenization (e.g., BPE, WordPiece): Instead of failing, the model splits rare or new words like "unhappiness" into known parts ("un", "happy", "ness").
- Character-Level Encoding: In extreme cases, the model breaks tokens down to individual characters to build a representation.
- Contextual Understanding: Even if a word is new, the LLM uses surrounding text (context) to infer meaning based on common word patterns, prefixes, and suffixes.
- Fine-Tuning: To improve performance on domain-specific vocabulary (e.g., medical or legal jargon), models are fine-tuned to familiarize themselves with these terms.

This approach allows modern models to handle typos, new slang, or technical jargon effectively, though it may make processing slower and increase token counts for unusual words.
...
- Vulnerable to producing biased or harmful content.
