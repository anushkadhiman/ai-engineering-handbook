...
Because MQA can sometimes hurt model quality too much, researchers (notably in Llama 3 and Mistral) use Grouped-Query Attention (GQA).
	• GQA is a compromise: instead of 1 shared head for everyone, you divide Query heads into groups, and each group shares one KV pair.
	• MHA: 8 Q, 8 K, 8 V
	• GQA: 8 Q, 2 K, 2 V (Balanced)
	• MQA: 8 Q, 1 K, 1 V (Extreme)
...
Key Aspects of Flash Attention:
IO-Awareness: By reducing the need to frequently access slower HBM, Flash Attention improves speed and reduces memory usage, acting as a "fused kernel" to perform calculations directly on the GPU.
Tiling Technique: It breaks down the large attention matrix into smaller blocks, allowing computations to stay within the faster, on-chip SRAM.
Exact Attention: It does not approximate results, ensuring the same output as standard attention.
Efficiency Gains: It typically delivers 2-4x faster,, training, and 15-40% faster overall model performance (e.g., Llama 2).
...
Why do we use it?
  Eliminates "Stragglers": In static batching, if one request in a batch takes 500 tokens to generate and another takes 10, the finished request stays "locked" in the GPU, wasting resources until the longest request is done.
  Reduces Padding: Static batching requires padding all sequences to the length of the longest one in the batch. Continuous batching processes only the actual tokens needed for each request.
  Massive Throughput: It can increase the number of tokens processed per second by 10x to 23x compared to naive batching. 
How it works
...
These methods optimize the mathematical operations within the Transformer architecture.
	• FlashAttention (1, 2, & 3): An "exact" attention algorithm that tiles the computation to fit into fast on-chip SRAM, reducing slow memory (VRAM) reads/writes by 10–20x.
