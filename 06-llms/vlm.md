...
2. Shifted Window Attention (The Core Secret)
   ...
   patches). Attention is only computed inside these windows. This reduces complexity from quadratic (
   ) to linear (
...
The Logic: Learning by Reconstruction
...
Bounding Box: Where exactly is it? (
...
Everything else is forced to predict "nothing."
Result: The model naturally learns to predict exactly one box per object.
Pros and Cons
Pro: Streamlined Pipeline. It removes the need for complex, manual computer vision engineering (no more anchors or NMS).
Pro: Global Context. Because it uses Transformers, it's great at understanding large objects that span the whole image.
Con: Slow Convergence. Training a DETR from scratch takes much longer than traditional models like YOLO.
Con: Small Objects. Original DETR struggled with tiny objects, though newer versions like Deformable DETR have fixed this.
