...
In the context of diffusion models, the variance schedule is the "blueprint" for how we destroy the data. It defines exactly how much noise is added at each discrete step of the process.
...
The variance schedule is a sequence of values denoted by (beta), where ranges from to (often).
As we saw in the forward equation:
• Scaling the Data (): This slightly shrinks the original image's signal so that the total variance doesn't explode as we add noise.
• Adding the Noise (): This defines the variance of the Gaussian noise being injected.

**Common Schedule Types**
Choosing how changes over time is a critical hyperparameter. There are three common strategies:
...
In a diffusion model, the network (U-Net) is a multi-tasker. It has to know how to denoise an image that is 99% noise () and an image that is 1% noise (). Since the weights of the neural network are shared across all steps, we use Time Embeddings to tell the model exactly where it is in the process.
...
Control Type What it looks at Use Case
• Canny / SoftEdge Outlines and edges. Turning a sketch into a photo.
