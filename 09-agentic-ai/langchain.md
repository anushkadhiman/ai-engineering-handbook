...
In LangChain, Document Loaders act as the "ingestion engine," standardising various data sources—from local files to research databases—into a uniform Document object. Each Document contains page_content (the text) and metadata (source details like filename or page number).

Core File & Web Loaders
Loader Class Name Best For
PDF PyPDFLoader Standard PDFs; treats each page as a separate document.
Web WebBaseLoader Scraping text from URLs using BeautifulSoup/urllib.
CSV CSVLoader Loading each row as a separate document.
Excel UnstructuredExcelLoader Extracts raw text from .xlsx and .xls files.
Word Docx2txtLoader Standard text extraction from .docx files.
PowerPoint UnstructuredPowerPointLoader Converts presentations into searchable text.
TXT TextLoader Simple, direct loading of plain text files.
JSON JSONLoader Uses jq schema to target specific keys for extraction.
Specialised & AI-Powered Loaders
Arxiv Loader: ArxivLoader fetches full text and metadata for academic papers directly via the Arxiv API.
UpstageDocumentParseLoader: A high-precision Upstage integration that excels at complex document layouts, offering lazy_load for memory efficiency.
LlamaParse: An advanced parser by LlamaIndex (often used with LangChain) designed to handle tables and intricate formatting that standard loaders might miss.
HWP (Hangeul) Loader: Essential for Korean document formats (.hwp), typically handled through specialized loaders like HwpLoader within the Korean community packages.
...
4. Specialized Retrievers
   ...
   Kiwi BM25: A specialized version of the BM25 algorithm (keyword search) optimized for Korean text, ensuring better tokenization and keyword matching for that language.
...
1. Core Features (The "Brain" of the Graph)
   ...
   Checkpoints (Persistence): LangGraph can automatically save the state after every step. This enables Human-in-the-loop interactions (pausing the agent for approval) and Time Travel (reverting to a previous state to debug).
   Nodes & Edges:
